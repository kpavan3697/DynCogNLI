...
if __name__ == "__main__":
    ...
    # Correct the default path to match your actual file name
    # The default value should match your file: train.csv
    parser.add_argument("--atomic", type=str, default="data/atomic2020/train.csv", help="Path to the ATOMIC file.")
    ...
    parser.add_argument("--patience", type=int, default=DEFAULT_PATIENCE, help="Number of epochs to wait for validation loss improvement.")
    ...
    class EarlyStopping:
        ...
        def __call__(self, val_loss, model):
            if self.best_loss is None:
                ...
                if self.verbose:
                    print(f"Validation loss improved ({self.best_loss:.4f} --> {val_loss:.4f}). Saving model state...")
                self.best_loss = val_loss
                self.best_model_state = model.state_dict()
                self.counter = 0
    ...
    # Now, the logic correctly uses the path from `args.atomic`
    if os.path.exists(args.atomic):
        atomic_triples = load_atomic_tsv(args.atomic, max_triples=DEFAULT_MAX_ATOMIC)
    else:
        # The print statement now correctly reports the path that was tried
    ...
    # Use a while loop instead of a fixed number of epochs
    epoch = 0
    while not early_stopping.early_stop:
        ...
        if early_stopping.early_stop:
            print("[train] Early stopping triggered. Training stopped.")
